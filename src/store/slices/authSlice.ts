...
export const doLoginGG = createAsyncThunk('auth/doLoginGG', async (loginPayload: any, { dispatch }) =>
  loginGG(loginPayload).then((res) => {
    dispatch(setUser(res.user));
    persistToken(res.token);
...
const authSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(doLoginGG.fulfilled, (state, action) => {
      state.token = action.payload;
    });
